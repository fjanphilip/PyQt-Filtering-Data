...
class MainWindow(QMainWindow):
    ...
    # ///////////////////////////////////////////////////////////////
    # def coba(self):
    #     print("btn_cari_file_ditekannn!!")
    # def coba2(self):
    #     print("btn_load_file_ditekan")  
    # path_data = []
    ...
    def filter_data(self):
        ...
        try:
            ...
            nama_kolom = df.columns.to_list()

            # Inisialisasi daftar untuk menyimpan data hasil filter
            filtered_data = []

            index_def = df[(df['Product'] == "PERTAMAX") & (df['Volume'] == 2000)].index
            not_filtered_data = df.drop(index_def)
            filtered_data_value = df[(df['Product'] == "PERTAMAX") & (df['Volume'] == 2000)]
            selected_columns = ['Spbu No.', 'Ship To Code', 'OrderNumber', 'Delivery Number', 'Product', 
            ...
            # Lakukan filter 
            filtered_data = df.query('Product == "PERTAMAX" and Volume == 2000').values.tolist()

            # Buat model baru untuk QTableView
            model = QStandardItemModel(len(filtered_data), len(nama_kolom))
            model.setHorizontalHeaderLabels(nama_kolom)

            # Masukkan data yang sudah difilter ke dalam model
            for row_idx, row_data in enumerate(filtered_data):
                for col_idx, value in enumerate(row_data):
                    item = QStandardItem(str(value))
                    model.setItem(row_idx, col_idx, item)

            # Set model ke QTableView yang sesuai
            self.ui.load_pages.table_view_data_2.setModel(model)
            # QMessageBox.information(self, "Sukses", f"Data berhasil difilter! {len(filtered_data)} baris ditemukan.")
    ...
    def filter_data_3(self):
        ...
        print("Path Mentah:", path_mentah)
        print("Path Filter:", path_filter)
    ...
    def export_data_3(self):
        ...
        try:
            with pd.ExcelWriter(file_path, engine='xlsxwriter') as writer:
                ...
                rekap_data = []
                print(filtered_out_data.columns.to_list())
                ...
                pivot_startcol = len(filtered_out_data.columns) + 3
                rekap_startrow = 10
                rekap_startcol = 14
